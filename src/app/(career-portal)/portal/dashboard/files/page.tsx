...
interface FileItem {
  ...
  fileType: string; mimeType: string; createdAt: string;
}
      ...
      <header className="bg-white/90 backdrop-blur-md border-b border-slate-200 sticky top-0 z-10 shadow-sm">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
          ...
          {!loading && files.length > 0 && (
            <span className="ml-auto text-xs text-slate-400">{files.length} file{files.length !== 1 ? 's' : ''}</span>
          )}
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 py-7 space-y-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Download and save all files. Use the revision button if you need changes.
          </p>
        </div>
          ...
          <>
              ...
              {files.map(file => {
                ...
                return (
                  <div key={file.id}
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <button
                          ...
                          {copying === file.id ? (
                            ...
                            <svg width="14" height="14" fill="none" viewBox="0 0 24 24">
                              <path stroke="currentColor" strokeWidth="2" strokeLinecap="round" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3"/>
                            </svg>
                          )}
                        </button>
                        <a href={file.fileUrl} target="_blank" rel="noopener noreferrer"
                          ...
                          title="Preview">
                          <svg width="14" height="14" fill="none" viewBox="0 0 24 24">
                            <path stroke="currentColor" strokeWidth="2" strokeLinecap="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke="currentColor" strokeWidth="2" strokeLinecap="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                          </svg>
                        </a>
                        <a href={file.fileUrl} download
                          ...
                          <svg width="12" height="12" fill="none" viewBox="0 0 24 24">
                            <path stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                          </svg>
                          Download
                        </a>
                      </div>
                    ...
                    {/* Request revision row */}
                    <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between">
                      <p className="text-xs text-slate-400">Need changes to this file?</p>
                      <button
                        onClick={() => openRevision(file.label)}
                        className="flex items-center gap-1.5 text-xs font-semibold text-orange-600 hover:text-orange-700 border border-orange-200 hover:border-orange-300 px-2.5 py-1 rounded-lg transition-colors"
                      >
                        <svg width="12" height="12" fill="none" viewBox="0 0 24 24">
            ...
            <div className="px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3">
              ...
              <p className="text-xs text-amber-800 leading-relaxed">
                <strong>Tip:</strong> Save all files to Google Drive, Dropbox, or your local computer. Hosted links remain
                active for 30 days — always keep a local copy.
              </p>
            </div>
          </>
        )}
