// src/app/api/admin/career/[id]/settle/route.ts
// PATCH — Set settlement data on a manual Career client record.
...
export async function PATCH(
  ...
  if (!client) return NextResponse.json({ error: 'Career client not found' }, { status: 404 });

  const updated = await db.careerClient.update({
    where: { id },
    data: {
