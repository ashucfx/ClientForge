...
export async function GET(req: NextRequest) {
  try {
    ...
    const revenueMetrics = await db.flywheelRevenueMetrics.findMany({
      where: whereClause,
      orderBy: {
        date: 'desc'
      },
    ...
    console.error('[FlywheelRevenue] GET Error:', error);
    return NextResponse.json({ success: false, error: 'Failed to fetch revenue metrics' }, { status: 500 });
  }
}
