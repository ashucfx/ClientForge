...
// The page now receives pre-fetched templates as props from the server component.
// This avoids client-side fetch auth issues entirely.
export function NewProjectForm({ templatesByCat }: { templatesByCat: Record<string, any[]> }) {
  ...
  const categoryKeys = Object.keys(templatesByCat);
        ...
        <form onSubmit={handleSubmit} style={{ 
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24, flex: 1 }}>
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 8 }}>Service Blueprint <span style={{color: '#EF4444'}}>*</span></label>
                <select 
                  required 
                  className="input" 
                  style={{ 
                ...
                >
                  <option value="" disabled>
                    {categoryKeys.length === 0 ? 'No blueprints available — please seed templates' : 'Select a service blueprint...'}
                  </option>
                ...
                {categoryKeys.length > 0 && (
                  <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 8 }}>
                    {categoryKeys.length} categories · Instantly provisions milestones, tasks &amp; deliverables.
                  </p>
                )}
              </div>
